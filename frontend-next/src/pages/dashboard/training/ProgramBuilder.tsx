...
import { ArrowLeft, GripVertical, Plus, Trash2, CheckCircle2, Circle, Calendar } from 'lucide-react'
...
export default function ProgramBuilder() {
  ...
  const handleAddExercise = async (blockId: number, data: { name: string; sets: string; reps: string; load: string; rest_seconds: string }) => {
    try {
                        ...
                        <div className="mt-3 space-y-1.5">
                          {block.exercises.map((ex) => (
                            <div
                              ...
                              className={cn('flex items-center gap-3 rounded-lg bg-background px-3 py-2.5', isStaff && 'cursor-move')}
                            >
                              {isStaff ? (
...
function ExerciseForm({ blockId, onSubmit }: { blockId: number; onSubmit: (blockId: number, data: { name: string; sets: string; reps: string; load: string; rest_seconds: string }) => void }) {
  const [data, setData] = useState({ name: '', sets: '', reps: '', load: '', rest_seconds: '' })
  return (
    <div className="mt-3 grid grid-cols-2 gap-2 rounded-xl bg-white/5 p-4 sm:grid-cols-5">
